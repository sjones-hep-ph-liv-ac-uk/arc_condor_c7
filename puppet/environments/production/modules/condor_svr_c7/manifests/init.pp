class condor_svr_c7() inherits condor_svr_c7::params 
{
  ...
  # - REPOS
  #yumrepo { "condor_ c7_svr":
  #  descr => "condor_ c7_svr",
  ...
  file { "/etc/ld.so.conf.d/condor.conf":
    source => "puppet:///modules/condor_svr_c7/condor.conf",
    mode => "644",
  }
  exec { "/sbin/ldconfig":
  ...
  file { "/etc/condor/config.d/00-headnode_parameters":
    source => "puppet:///modules/condor_svr_c7/00-headnode_parameters",
    mode => "644",
  }

  file { "/etc/condor/config.d/14accounting-groups-map.config":
    source => "puppet:///modules/condor_svr_c7/14accounting-groups-map.config",
    mode => "644",
  }
  file { "/etc/condor/config.d/11fairshares.config":
    source => "puppet:///modules/condor_svr_c7/11fairshares.config",
    mode => "644",
  }
  ...
  file { '/etc/condor/condor_config.local':
    ...
    content => template('condor_svr_c7/condor_config.local.erb'),
  }
  
  # Condor
  file { '/etc/condor/config.d': ensure  => directory, mode    => '755', }
  file { "/etc/condor/pool_password": source => "puppet:///modules/condor_svr_c7/pool_password" ,  mode    => '400', }
